"""Source names should not have spaces, use _ instead
search for spaces in vim via:
        :?\ 
        :set hlseach
        :set nohlsearch
"""

source_names = ['ESO_243-49_HLX-1',         
                'NAME_Holmberg_IX_X-1',
                'Holmberg_II_X-1',
                'M31_ULX-1',
                '[LM2005]_NGC_598_ULX1',    # AKA M33 X-8
                'RX_J133001+47137',         # M51 ULX-7
                '[LM2005]_NGC_3031_ULX1',
                'M82_X-2',
                '[LM2005]_NGC_1042_ULX1',
                'NAME_NGC_1313_X-1',
                'NAME_NGC_1313_X-2',
                '[LM2005]_NGC_247_ULX1',
                '[LB2005]_NGC_253_X20',     # NGC 253 ULX1
                'NAME_NGC_300_ULX1',
                '[LM2005]_NGC_4395_ULX1',
                '[LM2005]_NGC_5204_ULX1',
                '[LM2005]_NGC_5408_ULX1',
                '[SRW2006b]_NGC_55_ULX',
                '[SST2011]_J141939.39+564137.8', # NGC 5585 ULX (overlapping sources)
                'NAME_NGC_5907_ULX',
                '[LB2005]_NGC_6946_ULX1',
                '3XMM_J213631.9-543357', # NGC 7090
                'NAME_NGC_7793_P13',
                'NAME_NGC_925_ULX-1',
                'NAME_NGC_925_ULX-2',
                'SMC_X-3',
                'SS433',
                'SWIFT_J0243.6+6124',
                'NAME_UGC_6456_ULX',
                'NOVA_Cyg_1989',          # v404 Cygni
                '[LB2005]_NGC_5236_ULX1', # Outer ULX M83
                'NAME_M83_ULX-1',         # Inner ULX M83
                'NAME_NGC_1365-X1',
                '[SK2009]_X2',             #1365 X-2
                'NAME_IC_10_X-1',
                '[WMR2006]_NGC4945_XMM1',
                '[LB2005]_NGC_3031_X6']
